...
static int is_exception_image_grid(int global_offset)
{
    ...
    // road to granary: in julius the dirt roads lead into the granary
    if (both_between(v1, v2, 591, 639)) {
        return 1;
    }
    // road to granary: in julius the paved roads also lead into the granary
    if (both_between(v1, v2, 640, 657)) {
        return 1;
    }
    return 0;
}
