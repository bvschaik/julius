...
static void draw_scenario_info(void)
{
    ...
    image_draw(image_group(GROUP_SCENARIO_IMAGE) + data.info.image_id, 78, 36);
    ...
    if (data.show_minimap) {
        widget_minimap_draw_from_buffer(332, 119, 286, 300, data.info.minimap_image);
        ...
        // military
        int num_invasions = data.info.total_invasions;
        if (num_invasions <= 0) {
            text_id = 112;
