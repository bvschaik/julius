...
static void window_building_draw_monument_resources_needed(building_info_context* c) {
    ...
    for (int r = RESOURCE_TIMBER; r <= RESOURCE_MARBLE; r++) {
        int total_resources_needed = building_monument_resources_needed_for_monument_type(b->type, r, b->data.monument.monument_phase);
...
static void window_building_draw_monument_construction_process(building_info_context* c, int tr_phase_name, int tr_phase_name_text, int tr_construction_desc) {
    ...
    if (b->data.monument.monument_phase != MONUMENT_FINISHED) {
        if (!c->has_road_access) {
            window_building_draw_description_at(c, 55, 69, 25);
            text_draw_multiline(translation_for(tr_construction_desc), c->x_offset + 22, c->y_offset + 180, 16 * (c->width_blocks - 4), FONT_NORMAL_BLACK, 0);
            return;
        }
        int width = text_draw(translation_for(TR_CONSTRUCTION_PHASE), c->x_offset + 22, c->y_offset + 50, FONT_NORMAL_BLACK, 0);
        width += text_draw_number(b->data.monument.monument_phase, '@', "/", c->x_offset + 22 + width, c->y_offset + 50, FONT_NORMAL_BLACK);
        width += text_draw_number(building_monument_monument_phases(b->type) - 1, '@', "", c->x_offset + 10 + width, c->y_offset + 50, FONT_NORMAL_BLACK);
        text_draw(translation_for(tr_phase_name + b->data.monument.monument_phase - 1), c->x_offset + 32 + width, c->y_offset + 50, FONT_NORMAL_BLACK, 0);
        ...
        int height = text_draw_multiline(translation_for(tr_phase_name_text + b->data.monument.monument_phase - 1), c->x_offset + 22, c->y_offset + 170, 16 * (c->width_blocks - 4), FONT_NORMAL_BLACK, 0);
        ...
        if (c->height_blocks > 26) {
            ...
            image_draw(assets_get_image_id(assets_get_group_id("Areldir", "UI_Elements"), "Const. Banner 01") + phase_offset, c->x_offset + 32, c->y_offset + 196 + height);
        }
    }
}
...
static void window_building_draw_monument_colosseum_construction_process(building_info_context* c) {
    window_building_draw_monument_construction_process(c, TR_BUILDING_COLOSSEUM_PHASE_1, TR_BUILDING_COLOSSEUM_PHASE_1_TEXT, TR_BUILDING_MONUMENT_CONSTRUCTION_DESC);
}

static void window_building_draw_monument_hippodrome_construction_process(building_info_context* c) {
    window_building_draw_monument_construction_process(c, TR_BUILDING_HIPPODROME_PHASE_1, TR_BUILDING_HIPPODROME_PHASE_1_TEXT, TR_BUILDING_MONUMENT_CONSTRUCTION_DESC);
}
...
void window_building_draw_lararium(building_info_context* c)
{
    ...
    window_building_play_sound(c, "wavs/oracle.wav");
    outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
    text_draw_centered(translation_for(TR_BUILDING_LARARIUM), c->x_offset, c->y_offset + 10, 16 * c->width_blocks, FONT_LARGE_BLACK, 0);
...
{
    draw_grand_temple(c, "wavs/temple_farm.wav", TR_BUILDING_GRAND_TEMPLE_CERES_DESC, TR_BUILDING_GRAND_TEMPLE_CERES_BONUS_DESC, assets_get_image_id(assets_get_group_id("Areldir", "UI_Elements"), "Ceres L Banner"), TR_BUILDING_CERES_TEMPLE_QUOTE, GOD_CERES, 0);
}

void window_building_draw_grand_temple_neptune(building_info_context* c)
{
    draw_grand_temple(c, "wavs/temple_ship.wav", TR_BUILDING_GRAND_TEMPLE_NEPTUNE_DESC, TR_BUILDING_GRAND_TEMPLE_NEPTUNE_BONUS_DESC, assets_get_image_id(assets_get_group_id("Areldir", "UI_Elements"), "Nept L Banner"), TR_BUILDING_NEPTUNE_TEMPLE_QUOTE, GOD_NEPTUNE, 0);
}

void window_building_draw_grand_temple_mercury(building_info_context* c)
{
    draw_grand_temple(c, "wavs/temple_comm.wav", TR_BUILDING_GRAND_TEMPLE_MERCURY_DESC, TR_BUILDING_GRAND_TEMPLE_MERCURY_BONUS_DESC, assets_get_image_id(assets_get_group_id("Areldir", "UI_Elements"), "Merc L Banner"), TR_BUILDING_MERCURY_TEMPLE_QUOTE, GOD_MERCURY, 0);
}

void window_building_draw_grand_temple_mars(building_info_context* c)
{
    draw_grand_temple(c, "wavs/temple_war.wav", TR_BUILDING_GRAND_TEMPLE_MARS_DESC, TR_BUILDING_GRAND_TEMPLE_MARS_BONUS_DESC, assets_get_image_id(assets_get_group_id("Areldir", "UI_Elements"), "Mars L Banner"), TR_BUILDING_MARS_TEMPLE_QUOTE, GOD_MARS, 50);
}

void window_building_draw_grand_temple_venus(building_info_context* c)
{
    draw_grand_temple(c, "wavs/temple_love.wav", TR_BUILDING_GRAND_TEMPLE_VENUS_DESC, TR_BUILDING_GRAND_TEMPLE_VENUS_BONUS_DESC, assets_get_image_id(assets_get_group_id("Areldir", "UI_Elements"), "Venus L Banner"), TR_BUILDING_VENUS_TEMPLE_QUOTE, GOD_VENUS, 20);
}

void window_building_draw_pantheon(building_info_context* c)
{
    draw_grand_temple(c, "wavs/oracle.wav", TR_BUILDING_PANTHEON_DESC, TR_BUILDING_PANTHEON_BONUS_DESC, assets_get_image_id(assets_get_group_id("Areldir", "UI_Elements"), "Panth L Banner"), TR_BUILDING_PANTHEON_QUOTE, GOD_PANTHEON, 0);
}
...
void window_building_draw_tavern(building_info_context* c)
{
    ...
    text_draw_number(b->data.market.inventory[INVENTORY_WINE], '@', " ", c->x_offset + 64, c->y_offset + 66, FONT_NORMAL_BLACK);

    image_draw(image_group(GROUP_RESOURCE_ICONS) + RESOURCE_MEAT + resource_image_offset(RESOURCE_MEAT, RESOURCE_IMAGE_ICON),
        c->x_offset + 142, c->y_offset + 60);
    text_draw_number(b->data.market.inventory[INVENTORY_MEAT], '@', " ", c->x_offset + 174, c->y_offset + 66, FONT_NORMAL_BLACK);

    if (!c->has_road_access) {
        window_building_draw_description_at(c, 69, 25, 96);
    }
    else if (b->num_workers <= 0) {
        text_draw_multiline(translation_for(TR_BUILDING_TAVERN_DESC_1), c->x_offset + 32, c->y_offset + 96, 16 * (c->width_blocks - 4), FONT_NORMAL_BLACK, 0);
    }
    else if (!b->data.market.inventory[INVENTORY_WINE]) {
        text_draw_multiline(translation_for(TR_BUILDING_TAVERN_DESC_2), c->x_offset + 32, c->y_offset + 96, 16 * (c->width_blocks - 4), FONT_NORMAL_BLACK, 0);
    }
    else if (!b->data.market.inventory[INVENTORY_MEAT]) {
        text_draw_multiline(translation_for(TR_BUILDING_TAVERN_DESC_3), c->x_offset + 32, c->y_offset + 96, 16 * (c->width_blocks - 4), FONT_NORMAL_BLACK, 0);
    }
    else {
        text_draw_multiline(translation_for(TR_BUILDING_TAVERN_DESC_4), c->x_offset + 32, c->y_offset + 96, 16 * (c->width_blocks - 4), FONT_NORMAL_BLACK, 0);
    }
...
void window_building_draw_lighthouse(building_info_context* c) {
    ...
    if (b->data.monument.monument_phase == MONUMENT_FINISHED) {
        ...
        image_draw(assets_get_image_id(assets_get_group_id("Areldir", "UI_Elements"), "Lighthouse Banner"), c->x_offset + 32, c->y_offset + 166 + height);
...
void window_building_draw_grand_garden(building_info_context* c)
{
    ...
    outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
    inner_panel_draw(c->x_offset + 16, c->y_offset + 136, c->width_blocks - 2, 4);
    text_draw_centered(translation_for(TR_BUILDING_GRAND_GARDEN), c->x_offset, c->y_offset + 12, 16 * c->width_blocks, FONT_LARGE_BLACK, 0);
}
...
void window_building_draw_nymphaeum(building_info_context* c)
{
    ...
    if (b->data.monument.monument_phase <= 0) {
        window_building_play_sound(c, "wavs/oracle.wav");
        outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
        text_draw_centered(translation_for(TR_BUILDING_NYMPHAEUM), c->x_offset, c->y_offset + 12, 16 * c->width_blocks, FONT_LARGE_BLACK, 0);
        text_draw_multiline(translation_for(TR_BUILDING_NYMPHAEUM_DESC), c->x_offset + 22, c->y_offset + 56, 15 * c->width_blocks, FONT_NORMAL_BLACK, 0);
        inner_panel_draw(c->x_offset + 16, c->y_offset + 146, c->width_blocks - 2, 4);
        window_building_draw_employment(c, 152);
    } else {
        outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
...
void window_building_draw_small_mausoleum(building_info_context* c)
{
    ...
    if (b->data.monument.monument_phase <= 0) {
        window_building_play_sound(c, "wavs/oracle.wav");
        outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
        text_draw_centered(translation_for(TR_BUILDING_SMALL_MAUSOLEUM), c->x_offset, c->y_offset + 12, 16 * c->width_blocks, FONT_LARGE_BLACK, 0);
        text_draw_multiline(translation_for(TR_BUILDING_SMALL_MAUSOLEUM_DESC), c->x_offset + 22, c->y_offset + 56, 15 * c->width_blocks, FONT_NORMAL_BLACK, 0);
        inner_panel_draw(c->x_offset + 16, c->y_offset + 146, c->width_blocks - 2, 4);
        window_building_draw_employment(c, 152);
    } else {
        outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
...
void window_building_draw_large_mausoleum(building_info_context* c)
{
    ...
    if (b->data.monument.monument_phase <= 0) {
        window_building_play_sound(c, "wavs/oracle.wav");
        outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
        text_draw_centered(translation_for(TR_BUILDING_LARGE_MAUSOLEUM), c->x_offset, c->y_offset + 12, 16 * c->width_blocks, FONT_LARGE_BLACK, 0);
        text_draw_multiline(translation_for(TR_BUILDING_LARGE_MAUSOLEUM_DESC), c->x_offset + 22, c->y_offset + 56, 15 * c->width_blocks, FONT_NORMAL_BLACK, 0);
        inner_panel_draw(c->x_offset + 16, c->y_offset + 146, c->width_blocks - 2, 4);
        window_building_draw_employment(c, 152);
    } else {
        outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
