...
static void toggle_resource_state(int index, int param2)
{
    ...
    int resource;
    if (b->type == BUILDING_MARKET || b->type == BUILDING_DOCK || b->type == BUILDING_MESS_HALL || building_is_ceres_temple(b->type) || building_is_venus_temple(b->type) || b->type == BUILDING_TAVERN) {
