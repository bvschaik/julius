...
static void draw_housing_table()
{
    ...
    for (int i = 0; i <= 19; i++) {
        if (housing_type_counts[i]) {
            ...
            text_draw_number(housing_type_counts[i], '@', " ", x + 180, y_offset + (20 * rows), FONT_NORMAL_WHITE);
            if (rows == 11) {
                x += 280;
                rows = 0;
            } else {
...
static int draw_background(void)
{
    if (!housing_advisor_image) {
        housing_advisor_image = assets_get_image_id(assets_get_group_id("MSTVD", "UI_Elements"),
    ...
    image_draw(housing_advisor_image, 10, 10);
