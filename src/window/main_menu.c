...
static int editor_button_enabled = 0;
...
static generic_button buttons[] = {
    ...
    {192, 260, 448, 285, GB_IMMEDIATE, button_click, button_none, 5, 0},
};
...
{
    int num_buttons = editor_button_enabled ? 5 : 4;
    generic_buttons_handle_mouse(mouse_in_dialog(m), 0, 0, buttons, num_buttons, &focus_button_id);
}
...
static void button_click(int type, int param2)
{
    if (type == 1) {
        ...
        game_init_editor();
    } else if ((editor_button_enabled && type == 5) || (!editor_button_enabled && type == 4)) {
...
void window_main_menu_show(void)
{
    ...
    editor_button_enabled = editor_is_present();
    window_show(&window);
}
