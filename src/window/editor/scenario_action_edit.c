...
{
    data.action = action;
    data.xml_info = scenario_events_parameter_data_get_actions_xml_attributes(action->type);
}
...
static void draw_foreground(void)
{
    ...
    text_draw_centered(translation_for(TR_EDITOR_SCENARIO_ACTION), 32, 32, BUTTON_WIDTH, FONT_LARGE_BLACK, 0);
    ...
    if (data.xml_info->xml_parm1.type > PARAMETER_TYPE_UNDEFINED) {
        ...
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm1.name), data.action->parameter1, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    ...
    if (data.xml_info->xml_parm2.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm2.name), data.action->parameter2, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;
    ...
    if (data.xml_info->xml_parm3.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm3.name), data.action->parameter3, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;
    ...
    if (data.xml_info->xml_parm4.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm4.name), data.action->parameter4, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;
    ...
    if (data.xml_info->xml_parm5.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm5.name), data.action->parameter5, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;
...
static void set_amount_param(int value)
{
    switch (data.target_param) {
        case 1: data.action->parameter1 = value; break;
        case 2: data.action->parameter2 = value; break;
        case 3: data.action->parameter3 = value; break;
        case 4: data.action->parameter4 = value; break;
        case 5: data.action->parameter5 = value; break;
    }
}
