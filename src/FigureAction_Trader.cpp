    ...
    case FigureActionState_102_TradeCaravanTrading:
        ...
        if (f->waitTicks > 10)
        {
            ...
            if (FigureAction_TradeCaravan_canBuy(figureId, f->destinationBuildingId, f->empireCityId))
            {
                ...
                if (resource)
                {
                    trade_route_increase_traded((resource_type)Data_Empire_Cities[f->empireCityId].routeId, (resource_type)resource);
            ...
            if (FigureAction_TradeCaravan_canSell(figureId, f->destinationBuildingId, f->empireCityId))
            {
                ...
                if (resource)
                {
                    trade_route_increase_traded(Data_Empire_Cities[f->empireCityId].routeId, (resource_type)resource);
