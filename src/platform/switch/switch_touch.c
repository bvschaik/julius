...
static void preprocess_finger_down(SDL_Event *event)
{
    ...
    if (touch_mode == TOUCH_MODE_DIRECT) {
        int x = event->tfinger.x * SWITCH_DISPLAY_WIDTH;
        int y = event->tfinger.y * SWITCH_DISPLAY_HEIGHT;
    ...
    for (int i = 0; i < MAX_NUM_FINGERS; i++) {
        ...
        finger[port][i].last_x = event->tfinger.x * SWITCH_DISPLAY_WIDTH;
        finger[port][i].last_y = event->tfinger.y * SWITCH_DISPLAY_HEIGHT;
        break;
    }
}
...
static void preprocess_finger_up(SDL_Event *event)
{
    ...
    for (int i = 0; i < MAX_NUM_FINGERS; i++) {
        ...
        if (!multi_finger_dragging[port]) {
            ...
            float xrel = ((event->tfinger.x * SWITCH_DISPLAY_WIDTH) - (finger[port][i].last_down_x * SWITCH_DISPLAY_WIDTH));
            float yrel = ((event->tfinger.y * SWITCH_DISPLAY_HEIGHT) - (finger[port][i].last_down_y * SWITCH_DISPLAY_HEIGHT));
...
void switch_finish_simulated_mouse_clicks()
{
