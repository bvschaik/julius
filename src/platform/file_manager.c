...
int platform_file_manager_get_directory_contents_by_extension(char **file_list, int *file_count, const char *extension, int max_files)
{
    ...
    while ((entry = fs_dir_read(d)) && num_files < max_files) {
        ...
        if (file_has_extension(name, extension)) {
            strncpy(file_list[num_files], name, FILE_NAME_MAX);
            file_list[num_files][FILE_NAME_MAX - 1] = 0;
            ++num_files;
        }
...
{
    // Android already checks for proper case in the Java side, so if it was not found before, it won't be now
#ifndef __ANDROID__
...
#else
    return NULL != platform_file_manager_case_correct_file(filename);
#endif
}
