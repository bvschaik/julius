...
static void write_type_data(buffer *buf, const building *b)
{
    ...
    } else if (b->type == BUILDING_MARKET || b->type == BUILDING_MESS_HALL || b->type == BUILDING_SMALL_TEMPLE_CERES || b->type == BUILDING_LARGE_TEMPLE_CERES || b->type == BUILDING_SMALL_TEMPLE_VENUS || b->type == BUILDING_LARGE_TEMPLE_VENUS || b->type == BUILDING_TAVERN) {
