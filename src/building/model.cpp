...
static int index_of(const char *haystack, uint8_t needle, int haystack_length)
{
    for (int i = 0; i < haystack_length; i++)
    {
...
static const char *get_value(const char *ptr, int filesize, int *value)
{
...
int model_load()
{
    ...
    int guard = 200;
    int brace_index;
    const char *ptr = (char*)&buffer[index_of_string(buffer, ALL_BUILDINGS, filesize)];
    do
    {
        guard--;
    ...
    int dummy;
    ptr = (char*)&buffer[index_of_string(buffer, ALL_BUILDINGS, filesize)];
    ...
    ptr = (char*)&buffer[index_of_string(buffer, ALL_HOUSES, filesize)];
