...
static void spawn_figure_warehouse(building *b)
{
    ...
    int x_road, y_road;
    if (Terrain_hasRoadAccess(b->x, b->y, b->size, &x_road, &y_road) ||
        Terrain_hasRoadAccess(b->x, b->y, 3, &x_road, &y_road)) {
...
static void spawn_figure_granary(building *b)
{
    check_labor_problem(b);
    int x_road, y_road;
    if (Terrain_hasRoadAccessGranary(b->x, b->y, &x_road, &y_road)) {
        spawn_labor_seeker(b, x_road, y_road, 100);
        if (has_figure_of_type(b, FIGURE_WAREHOUSEMAN)) {
            return;
        }
...
static void spawn_figure_hippodrome(building *b)
{
    ...
    int x_road, y_road;
    if (Terrain_hasRoadAccessHippodrome(b->x, b->y, &x_road, &y_road)) {
        if (b->housesCovered <= 50 || b->data.entertainment.days1 <= 0) {
            generate_labor_seeker(b, x_road, y_road);
        }
        int pct_workers = worker_percentage(b);
    ...
    if (Terrain_hasRoadAccess(b->x, b->y, b->size, &x_road, &y_road)) {
        ...
        if (b->figureId2) {
            ...
            if (dstBuildingId > 0) {
                ...
                if (Terrain_hasRoadAccess(bDst->x, bDst->y, bDst->size, &x_road, &y_road) ||
                    Terrain_hasRoadAccess(bDst->x, bDst->y, 3, &x_road, &y_road)) {
