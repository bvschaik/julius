...
int Empire_canExportResourceToCity(int cityId, int resource)
{
    ...
    if (cityId && trade_route_limit_reached(routeId, (resource_type)resource))
    {
        // quota reached
        return 0;
    }
...
int Empire_canImportResourceFromCity(int cityId, int resource)
{
    ...
    int routeId = Data_Empire_Cities[cityId].routeId;
    if (trade_route_limit_reached(routeId, (resource_type)resource))
    {
        return 0;
    }
    ...
    if (finishedGood)
    {
        maxInStock = 2 + 2 * building_count_industry_active((resource_type)finishedGood);
    }
