...
void crashHandler_handle_crash(int signum)
{
    ...
    {
    case SIGABRT:
        Logger::warning("SIGABRT: abort() called somewhere in the program.");
        break;
    case SIGSEGV:
        Logger::warning("SIGSEGV: Illegal memory access.");
        break;
    case SIGILL:
        Logger::warning("SIGILL: Executing a malformed instruction. (possibly invalid pointer)");
        break;
    case SIGFPE:
        Logger::warning("SIGFPE: Illegal floating point instruction (possibly division by zero).");
        break;
    }
...
void CrashHandler::printstack( bool show_message, unsigned int starting_frame, unsigned int max_frames )
{
#if !defined(JULIUS_PLATFORM_WIN) && !defined(JULIUS_PLATFORM_ANDROID)
    if (show_message)
    {
        std::string file = app.workdir() + "stdout.txt";
        std::string msg = fmt::format( "julius has crashed.\n\n"
                                       "A stacktrace has been written to:\n"
                                       "{}", file );
        app.errordlg( "julius: Unhandled exception", msg );
    }
