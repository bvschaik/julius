...
static void drawPaneling()
{
	...
	for (int x = data.xMin; x < data.xMax; x += 70) {
		...
		image_draw(graphicBase + 3, x, data.yMax - 40);
	}

	// horizontal bar borders
	for (int x = data.xMin; x < data.xMax; x += 86) {
		image_draw(graphicBase + 1, x, data.yMin);
		image_draw(graphicBase + 1, x, data.yMax - 120);
		image_draw(graphicBase + 1, x, data.yMax - 16);
	}

	// vertical bar borders
	for (int y = data.yMin + 16; y < data.yMax; y += 86) {
		image_draw(graphicBase, data.xMin, y);
		image_draw(graphicBase, data.xMax - 16, y);
	}

	// crossbars
	image_draw(graphicBase + 2, data.xMin, data.yMin);
	image_draw(graphicBase + 2, data.xMin, data.yMax - 120);
	image_draw(graphicBase + 2, data.xMin, data.yMax - 16);
	image_draw(graphicBase + 2, data.xMax - 16, data.yMin);
...
static void drawPanelInfo()
{
	...
	if (selectedObject) {
		...
		lang_text_draw_centered(47, 8, data.xMin, data.yMax - 48, data.xMax - data.xMin, FONT_NORMAL_GREEN);
	}
}
...
static void drawPanelInfoCity(const empire_object *object)
{
	...
	if (city->type == EMPIRE_CITY_DISTANT_ROMAN) {
		lang_text_draw_centered(47, 12, xOffset, yOffset + 42, 240, FONT_NORMAL_GREEN);
		return;
	}
	if (city->type == EMPIRE_CITY_VULNERABLE_ROMAN) {
		if (Data_CityInfo.distantBattleCityMonthsUntilRoman <= 0) {
			...
			lang_text_draw_centered(47, 13, xOffset, yOffset + 42, 240, FONT_NORMAL_GREEN);
		}
		return;
	}
	if (city->type == EMPIRE_CITY_FUTURE_TRADE ||
		...
		lang_text_draw_centered(47, 0, xOffset, yOffset + 42, 240, FONT_NORMAL_GREEN);
		return;
	}
	if (city->type == EMPIRE_CITY_OURS) {
		lang_text_draw_centered(47, 1, xOffset, yOffset + 42, 240, FONT_NORMAL_GREEN);
		return;
	}
	...
	if (city->is_open) {
		// city sells
		lang_text_draw(47, 10, xOffset + 40, yOffset + 30, FONT_NORMAL_GREEN);
		int goodOffset = 0;
		for (int good = 1; good <= 15; good++) {
			...
			switch (tradeMax) {
				case 15:
					image_draw(image_group(GROUP_TRADE_AMOUNT),
						xOffset + 100 * goodOffset + 141, yOffset + 20);
					break;
				case 25:
					image_draw(image_group(GROUP_TRADE_AMOUNT) + 1,
						xOffset + 100 * goodOffset + 137, yOffset + 20);
					break;
				case 40:
					image_draw(image_group(GROUP_TRADE_AMOUNT) + 2,
		...
		// city buys
		lang_text_draw(47, 9, xOffset + 40, yOffset + 60, FONT_NORMAL_GREEN);
		goodOffset = 0;
		for (int good = 1; good <= 15; good++) {
		...
		int goodOffset = lang_text_draw(47, 5, xOffset + 50, yOffset + 42, FONT_NORMAL_GREEN);
		for (int good = 1; good <= 15; good++) {
			if (!empire_object_city_sells_resource(object->id, good)) {
				continue;
			}
		...
		goodOffset += lang_text_draw(47, 4, xOffset + goodOffset + 100, yOffset + 42, FONT_NORMAL_GREEN);
		for (int good = 1; good <= 15; good++) {
			if (!empire_object_city_buys_resource(object->id, good)) {
				continue;
			}
		...
		goodOffset = lang_text_draw_amount(8, 0, city->cost_to_open,
			xOffset + 60, yOffset + 73, FONT_NORMAL_GREEN);
		lang_text_draw(47, 6, xOffset + goodOffset + 60, yOffset + 73, FONT_NORMAL_GREEN);
	}
}
...
static void drawPanelInfoRomanArmy(const empire_object *object)
{
	if (Data_CityInfo.distantBattleRomanMonthsToTravel > 0 ||
		...
		if (Data_CityInfo.distantBattleRomanMonthsTraveled == object->distant_battle_travel_months) {
			...
			lang_text_draw_multiline(47, textId, xOffset, yOffset, 240, FONT_NORMAL_BLACK);
		}
	}
}
...
{
	int graphicBase = image_group(GROUP_EMPIRE_PANELS);
	image_draw(graphicBase + 6, data.xMin + 2, data.yMax - 199);
	image_draw(graphicBase + 7, data.xMax - 84, data.yMax - 199);
	image_draw(graphicBase + 8, (data.xMin + data.xMax - 332) / 2, data.yMax - 181);
	if (city) {
...
static void drawEmpireObject(const empire_object *obj)
{
    ...
    image_draw(image_id, data.xDrawOffset + x, data.yDrawOffset + y);
...
static void drawEmpireMap()
{
	...
	image_draw(image_group(GROUP_EMPIRE_MAP), data.xDrawOffset, data.yDrawOffset);
...
static int getTooltipResource(struct TooltipContext *c)
{
	...
	if (city->is_open) {
		...
		int itemOffset = lang_text_get_width(47, 5, FONT_NORMAL_GREEN);
		for (int r = 1; r <= 15; r++) {
			if (empire_object_city_sells_resource(objectId, r)) {
		...
		itemOffset += lang_text_get_width(47, 4, FONT_NORMAL_GREEN);
		for (int r = 1; r <= 15; r++) {
			if (empire_object_city_buys_resource(objectId, r)) {
