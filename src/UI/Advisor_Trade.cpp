...
void UI_TradePricesDialog_drawBackground()
{
    ...
    for (int i = 1; i < 16; i++)
    {
        ...
        Widget_Text_drawNumberCentered(trade_price_buy((resource_type)i),
                                       baseOffsetX + 120 + 30 * i, baseOffsetY + 229, 30, FONT_SMALL_PLAIN);
        Widget_Text_drawNumberCentered(trade_price_sell((resource_type)i),
...
void UI_ResourceSettingsDialog_drawForeground()
{
    ...
    {
        int totalBuildings = building_count_industry_total((resource_type)selectedResourceId);
        int activeBuildings = building_count_industry_active((resource_type)selectedResourceId);
        if (building_count_industry_total((resource_type)selectedResourceId) <= 0)
        {
