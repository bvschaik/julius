...
static int checkEvolveDesirability(int buildingId)
{
    house_level level = Data_Buildings[buildingId].subtype.houseLevel;
...
static int hasRequiredGoodsAndServices(int buildingId, int forUpgrade)
{
    ...
    house_level level = b->subtype.houseLevel;
    if (forUpgrade)
    {
        level = (house_level)(level+1);
    }
    const model_house *model = model_get_house(level);
    // water
